...
fn draw_chart(
    ...
    let max_x = points.iter().fold(0.0_f64, |acc, p| acc.max(p.x)).max(1.0);
    let max_y = points.iter().fold(0.0_f64, |acc, p| acc.max(p.y)).max(1.0);
...

    {
        ...
        for (idx, point) in points.iter().enumerate() {
            let x = y_axis_x + (point.x / max_x) * plot_width;
            let y = x_axis_y - (point.y / max_y) * plot_height;
            ...
            geoms.push(PointGeometry { x, y, index: idx });
        }
    }
    ...
    let on_mouse_move = {
        ...
        Callback::from(move |event: MouseEvent| {
            ...
            for (idx, g) in geoms.iter().enumerate() {
                ...
                if dist2 <= 8.0_f64.powi(2) && best.is_none_or(|(_, b)| dist2 < b) {
                    best = Some((idx, dist2));
                }
            }

            if let Some((idx, _)) = best {
                hovered_index.set(Some(geoms[idx].index));
            ...
            />
            {
                if let (Some(idx), Some((x, y))) = (*hovered_index, *hover_pos) {
                    ...
                    html! {
                        ...
                        >
                            { format!("{}: ({:.0}, {:.0})", point.label, point.x, point.y) }
                        </div>
                    }
                } else {
                    Html::default()
